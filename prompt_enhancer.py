"""QwenVL Lean Prompt Enhancer — text-only GGUF model for prompt enhancement.

Refines/enhances VL output for better video generation.
Uses llama-cpp-python with text-only GGUF models (no mmproj).
"""
...
from .cache import PROMPT_CACHE, get_cache_key, save_cache
...
def _load_prompt_config():
    if not PROMPT_CONFIG_PATH.exists():
        raise FileNotFoundError(f"[QwenVL-Lean] Missing system_prompts.json at {PROMPT_CONFIG_PATH}")
    try:
        ...
        raise RuntimeError(f"[QwenVL-Lean] Failed to load system_prompts.json: {exc}") from exc
...
# ---------------------------------------------------------------------------
# Text model catalog loader
# ---------------------------------------------------------------------------

def _load_text_model_catalog():
    ...
    # Text-only catalog (Qwen_model, not qwenVL_model)
...
TEXT_MODEL_CATALOG = _load_text_model_catalog()
...
class QwenVL_Lean_PromptEnhancer:
    RETURN_TYPES = ("STRING",)
    ...
    def INPUT_TYPES(cls):
        ...
        temp = _load_text_model_catalog()
        model_keys = sorted(list((temp.get("models") or {}).keys())) or ["(edit gguf_models.json)"]
        default_model = model_keys[0]

        return {
            "required": {
                "model_name": (model_keys, {"default": default_model, "tooltip": "GGUF text model for prompt enhancement."}),
                "prompt_text": ("STRING", {"default": "", "multiline": True, "tooltip": "Prompt text to enhance. Leave blank to emit the preset instruction."}),
                ...
                "keep_model_loaded": ("BOOLEAN", {"default": True, "tooltip": "Keep model loaded for faster repeated inference."}),
    ...
    def _resolve_model_path(self, model_name):
        models = TEXT_MODEL_CATALOG.get("models") or {}
        ...
        base_dir = _resolve_base_dir(TEXT_MODEL_CATALOG.get("base_dir") or "LLM/GGUF")
    ...
    def _maybe_download_model(self, model_name, resolved):
        ...
        repo_ids = [rid for rid in [entry.get("repo_id")] + (entry.get("alt_repo_ids") or []) if rid]
        filename = entry.get("filename") or resolved.name
        if not repo_ids or not filename:
            raise FileNotFoundError(f"[QwenVL-Lean] GGUF missing and no repo_id/filename to download: {resolved}")

        target_dir = resolved.parent
        target_dir.mkdir(parents=True, exist_ok=True)
        for repo_id in repo_ids:
            print(f"[QwenVL-Lean] Downloading GGUF {filename} from {repo_id}")
            try:
        ...
        if not resolved.exists():
            raise FileNotFoundError(f"[QwenVL-Lean] GGUF model not found after download: {resolved}")
    ...
    def _load_model(self, model_name, device):
        ...
        model_cfg = TEXT_MODEL_CATALOG["models"].get(model_name, {})
        ...
        if not resolved.exists():
            raise FileNotFoundError(f"[QwenVL-Lean] GGUF model not found: {resolved}")

        print(f"[QwenVL-Lean] Loading text GGUF model from {resolved}")
        if device == "auto":
    ...
    def _invoke_llama(
        ...
        def _call(system: str, user: str, temp: float, seed_val: int) -> str:
            ...
            if not response or "choices" not in response or not response["choices"]:
                raise RuntimeError("[QwenVL-Lean] llama_cpp returned empty response")
        ...
        # If the model only emitted thinking/planning, do a single constrained retry.
    ...
    def process(
        ...
        if keep_last_prompt:
            if LAST_SAVED_PROMPT:
                print(f"[QwenVL-Lean Enhancer] Using last saved prompt")
        ...
        if cache_key in PROMPT_CACHE:
            ...
            if cached_text:
                print(f"[QwenVL-Lean Enhancer] Using cached prompt (seed={seed})")
                return (cached_text.strip(),)

        style_entry = STYLES.get(preset_system_prompt, {})
        ...
        if english_output:
            translated = self._invoke_llama(
                system_prompt=(
                    ...
                    or "Return a single English paragraph (150-300 words). No prefixes, bullets, JSON, or think tags. "
